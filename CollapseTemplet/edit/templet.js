window.$T = window.$t = (function ($w, $a) {
    ...
    return {
        ...
        createTemplet: function (params) {
            ...
            let viewModel = {
                ...
                refModels: {
                    tableRefModel: {
                        ...
                        initRefModel: function (appData) {
                            for (var i = 0; i < 50; i++) {
                                $app.viewModel.refModels.tableRefModel.refDataModel.push({
                                    id: "100" + i,
                                    code: "100" + i,
                                    age: 22,
                                    name: "张三" + i
                ...
                collapseModel : {
                    ...
                    items : [
                        ...
                        //     $model : null,                  // 关联数据模型
                        //     type: "from",                   // 类型（呈现为表格/表单）
                        //     title : "表单",                  // 显示标题
                        //     name : "dataForm",              // 作为 collapse-item 的 name
                        //     size: "mini",
                        //     display: true,
                        //     formModel: {
                        //         items: [],						// 表单域/表格列模型集合 (自定义)
                        //
            ...
            viewModel.toolbar.size = isNotNull(params.viewModel.toolbar.size) ? params.viewModel.toolbar.size : viewModel.size; // 工具条组件尺寸设置

            viewModel.collapseModel.accordion = isNotNull(params.viewModel.collapseModel.accordion) ? params.viewModel.collapseModel.accordion : viewModel.collapseModel.accordion;
            viewModel.collapseModel.spreadall = viewModel.collapseModel.accordion ? false : isNotNull(params.viewModel.collapseModel.spreadall) ? params.viewModel.collapseModel.spreadall : viewModel.collapseModel.spreadall;
            ...
            params.viewModel.collapseModel.items.forEach(function (collapseItemModel) {
                ...
                if(viewModel.collapseModel.spreadall) viewModel.collapseModel.value.push(collapseItemModel.name);
                ...
                var newCollapseItemModel = {
                    ...
                    $model : isNotNull(collapseItemModel.data_model_key) ? params.dataModel[collapseItemModel.data_model_key] : params.dataModel, // 关联数据模型
                    ...
                    size: isNotNull(collapseItemModel.size) ? collapseItemModel.size : viewModel.size,                                            //
                    display: isNotNull(collapseItemModel.display) ? collapseItemModel.display : true                                              //
                }
